...
架构核心价值为**可插拔**，越界即腐化。以下为所有开发（含改 core）的铁律。技术债与治理进度见 skill `core-dev`「技术债」（一类热点已全部收口为 Metadata 声明 + Core 默认兜底）。铁律含义：**新增/改动代码须按职责边界归位，禁止加深已知违反**；历史债务按排期治理，无需立即重构。
...
> 现状偏差：网关插件仍混合 provider+UI 职责；HostService 单通道过宽。错误格式/模型家族/调度映射等已改为插件 Metadata 声明（约定表见 skill `core-dev`），Core 仅保留默认兜底。新代码以此表为准，现有偏差按排期治理。
