...
3. **前置必读**：所在子项目 `CLAUDE.md` 及对应 skill（core→`core-dev`，插件→`develop-plugin`）。
...
8. **规范对得上代码（防漂移）**：改动涉及架构（分层/契约/转发/计费/调度/插件职责）须同步更新对应 skill。冲突即漂移，须修正。

## 任务路由

| 任务 | 文档 | Skill |
|---|---|---|
| core 开发（后端/前端/子系统） | `airgate-core/CLAUDE.md` | `core-dev` |
...
权威架构参考：skill `core-dev`。

## 生态边界（最重要，动手前必对照）

架构核心价值为**可插拔**，越界即腐化。以下为所有开发（含改 core）的铁律。技术债与治理进度见 skill `core-dev`「技术债」（错误格式/模型家族等已收口为 Metadata 声明 + Core 默认兜底；调度映射与图像 provider 路径仍存硬编码越界，已逐条登记，含越界判定标准）。铁律含义：**新增/改动代码须按职责边界归位，禁止加深已知违反**；历史债务按排期治理，无需立即重构。
...
> 现状偏差：网关插件仍混合 provider+UI 职责；HostService 单通道过宽。错误格式/模型家族等已收口为插件 Metadata 声明（约定表见 skill `core-dev`），Core 仅保留默认兜底；但**调度映射与图像 provider 路径仍存硬编码越界**（core 假定 OpenAI 为唯一图像/协议翻译 provider），已逐条登记 skill `core-dev`「技术债」「已登记的硬编码越界」，**勿加深**。新代码按约定表归位，现有偏差按排期治理。
...
> 部分插件仍为"网关+provider+UI"混合的过渡态。无需拆历史代码，但新增/改动须按职责归位、勿加深混合。归属存疑时先查职责速查表，再查 skill `core-dev`「技术债」。
