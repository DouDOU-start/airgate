...
description: 在 airgate-core 后端新增或修改 API 接口、领域业务逻辑、Ent schema 字段/表时使用。钉死 core 的分层落点与改动顺序（dto → handler → service → store → ent + 装配），并含 Ent 变更子流程，避免自创结构、漏装配、漏 make ent 导致 CI drift。
---

# core 后端开发（接口 / 领域逻辑 / Ent 变更）

airgate-core 后端是严格的 ports-and-adapters 分层。**别自创结构**——照下面的落点和顺序走。
...
| Service | `internal/app/<domain>/{service,types,errors}.go` | 业务逻辑；`Input/Filter/Result` 与 `Repository` 接口在 `types.go`；sentinel error 在 `errors.go` |
...
2. **Service**：在 `app/<domain>/types.go` 加 `Input/Filter/Result` 和 `Repository` 方法；`errors.go` 加 sentinel error；`service.go` 写业务逻辑（**不碰 gin/http，不 import ent**）。
...
3. **提交生成代码**（`ent/` 下生成部分）——漏提交会让 `make ci` 的 `verify-ent` drift 检查失败。
...
- handler 里硬编码 HTTP 状态码 / 把内部 err 直接返回前端：应走 service sentinel error（`errors.go`）+ handler 的 `handleError` 映射。⚠️ 上游账号失效用 **422 不用 401**（401 会踹掉管理员登录）。其余编码约定（slog 日志、凭证加密、ctx 透传、分页/时区）见 `airgate-core/CLAUDE.md`「后端编码约定」。
